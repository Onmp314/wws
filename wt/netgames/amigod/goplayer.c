...
const char *playReason;
short playLevel = 7;

/* procedure headers */
static short getMove (short *, short *);
static short checkPos (short, short, short);
static short takeCorner (short *, short *);
static short extend (short *, short *);
static short noNbrs (short, short);
static short extend2 (short *, short *);
static short lookForSave (short *, short *);
static short lookForSaveN (short *, short *);
static short lookForKill (short *, short *);
static short doubleAtari (short *, short *);
static short lookForAttack (short *, short *);
static short threaten (short *, short *);
static short connectCut (short *, short *);
static short heCanCut (short, short);
static short safeMove (short, short);
static short extendWall (short *, short *);
static short findAttack2 (short *, short *);
static short blockCut (short *, short *);
static short cutHim (short *, short *);
static short atariAnyway (short *, short *);
static short underCut (short *, short *);
static short dropToEdge (short *, short *);
static short pushWall (short *, short *);
static short reduceHisLiberties (short *, short *);
static short dropToEdge2 (short *, short *);

static short saveNLibs;
static pointList dapList1, dapList2, dapList3;
static short maxPlayLevel = 7;

static void genBord (color)
     enum bVal color;
{
     ...
     short *x, *y;
{
  ...
  for (i = 2; i <= maxPoint - 2; i++)
    if (claim[i][maxPoint - 2] == 0 && noNbrs (2, i))
    {
      *x = i;
      *y = maxPoint - 2;
      return TRUE;
    }
  for (i = maxPoint - 2; i >= 2; i--)
    if (claim[maxPoint - 2][i] == 0 && noNbrs (maxPoint - 2, i))
    {
      *x = maxPoint - 2;
      *y = i;
      return TRUE;
     ...
     short *x, *y;
{
  ...
  for (i = maxPoint - 3; i >= 3; i--)
    if (legal[maxPoint - 2][i])
    {
      value = claim[maxPoint - 2][i];
      if ((value < 7) &&
	  (value > -5) &&
	  (bord[maxPoint - 2][i + 1] == 0) &&
	  (bord[maxPoint - 2][i - 1] == 0))
	if (value < lowest)
	{
	  lowest = value;
  ...
  for (i = 3; i <= maxPoint - 3; i++)
    if (legal[i][maxPoint - 2])
    {
      value = claim[i][maxPoint - 2];
      if ((value < 7) &&
	  (value > -5) &&
	  (bord[i + 1][maxPoint - 2] == 0) &&
	  (bord[i - 1][maxPoint - 2] == 0))
	if (value < lowest)
	{
	  lowest = value;
	      ...
	      groupIDs[pList.p[2].px][pList.p[2].py])
	  {
		...
		{		/* protect point if possible */
		  ...
		  *y = j;
	      ...
	      /* 1-point extension, only protect if threatened */
	    {
	      if (((i > 0) && (ndbord[i - 1][j] == -1)) ||
		  ((j > 0) && (ndbord[i][j - 1] == -1)) ||
		  ((i < maxPoint) && (ndbord[i + 1][j] == -1)) ||
	      ...
	      groupIDs[pList.p[2].px][pList.p[2].py])
	  {
		...
		*y = j;
		if (safeMove (*x, *y))
	      ...
	      /* 1-point extension, only cut if connected */
	    {
	      if (connectMap[i][j] > 0)
	      {
		*x = i;
