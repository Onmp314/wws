...
static short me, him, depth, i, j, tryCount, tl, topMark, mark2;
static sPointList lList, dList;
static point tp;
static short libList[maxSPoint + 1];


static short mtNbrs (x, y)
     short x, y;
{				/* mtNbrs */
  short n = 0;
  if ((x > 0) && (bord[x - 1][y] == 0))
...
static short killTree (tx, ty, gx, gy, escape, tkMark)
...
static short tKillTree (tx, ty, gx, gy)
     short tx, ty, gx, gy;
{				/* tKillTree */
  short tkMark, escape;
  tryCount = 0;
